...
public class EmployerController {
	...
	public ResponseEntity<List<EmployerDTO>> getAllEmployers() {
		...
		List<EmployerDTO> dtos = employers.stream().map(dtoFactory::createDTO).collect(Collectors.toUnmodifiableList());
		return ResponseEntity.ok(dtos);
	}

	@PutMapping("/{id}")
	public ResponseEntity<EmployerDTO> updateEmployer(@PathVariable Long id, @RequestBody UpdateEmployerRequest req) {
	...
	public ResponseEntity<List<EmployerDTO>> getEmployersByDepartment(@PathVariable String department) {
	...
	public ResponseEntity<Set<EmployeeDTO>> getDirectReports(@PathVariable Long id) {
		...
		Set<EmployeeDTO> dtos = drs.stream().map(dtoFactory::createDTO).collect(Collectors.toUnmodifiableSet());
		return ResponseEntity.ok(dtos);
	}
