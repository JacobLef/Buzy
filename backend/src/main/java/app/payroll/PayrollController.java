...
public class PayrollController {
  ...
  public PayrollController(
    ...
    if (flatTaxStrategy == null) {
      ...
      logger.info(
          "PayrollController initialized with flatTaxStrategy: {}",
    ...
    if (progressiveTaxStrategy == null) {
      ...
      logger.info(
          "PayrollController initialized with progressiveTaxStrategy: {}",
   ...
   *
   * <p>POST /api/payroll/calculate/{employeeId} POST
  ...
  public ResponseEntity<PaycheckDTO> calculatePayroll(
      @PathVariable Long employeeId, @RequestParam(required = false) Double additionalPay) {
    if (additionalPay != null) {
      ...
      logger.info(
          "Payroll with additional pay calculated successfully for employee ID: {}", employeeId);
   ...
   *
   * <p>GET /api/payroll/preview/{employeeId} GET
  ...
  public ResponseEntity<PaycheckDTO> previewPayroll(
      @PathVariable Long employeeId, @RequestParam(required = false) Double additionalPay) {
    logger.info(
        "Received payroll preview request for employee ID: {} with additional pay: ${}",
        employeeId,
   ...
   *
   * <p>POST /api/payroll/bonuses Request Body: { "businessId": 1, "bonusAmount": 1000.00,
  ...
  public ResponseEntity<BonusDistributionResponse> distributeBonuses(
    ...
    logger.info(
        "Received bonus distribution request for business ID: {} with amount: ${}",
        request.businessId(),
        request.bonusAmount());
    ...
    BonusDistributionResponse response =
        BonusDistributionResponse.success(request.businessId(), businessName, paycheckDTOs);

    logger.info(
        "Bonus distribution completed successfully: {} paychecks generated", paycheckDTOs.size());
   ...
   *
   * <p>GET /api/payroll/tax-strategy
   *
   ...
   *
   * <p>GET /api/payroll/tax-strategies
   *
   ...
   * Switch tax strategy
   *
   * <p>PUT /api/payroll/tax-strategy?strategy=flatTaxStrategy PUT
      ...
      switch (strategy.toLowerCase()) {
        ...
        default:
          return ResponseEntity.badRequest()
              .body(
                  TaxStrategySwitchResponse.error(
                      "Invalid strategy. Use 'flatTaxStrategy' or 'progressiveTaxStrategy'"));
      }

      payrollService.setTaxStrategy(selectedStrategy);

      TaxStrategySwitchResponse response =
          TaxStrategySwitchResponse.success(
              "Tax strategy updated successfully", selectedStrategy.getStrategyName());
   ...
   *
   * <p>GET /api/payroll/history/{employeeId}
   *
   * @param employeeId ID of the employee
   ...
   *
   * <p>GET /api/payroll/history/{employeeId}?startDate=2024-01-01&endDate=2024-12-31
   *
   * @param employeeId ID of the employee
  ...
  public ResponseEntity<List<PaycheckDTO>> getPayrollHistoryByDateRange(
    ...
    logger.info(
        "Retrieved {} paychecks for employee ID: {} in date range", history.size(), employeeId);
   ...
   *
   * <p>GET /api/payroll/summary/{businessId}?startDate=2024-01-01&endDate=2024-12-31
   *
  ...
  public ResponseEntity<PayrollSummaryDTO> getPayrollSummary(
    ...
    logger.info(
        "Payroll summary generated: {} paychecks, {} employees, Total Net: ${}",
        summary.totalPaychecks(),
   ...
   *
   * <p>PUT /api/payroll/paycheck/{paycheckId} Request Body (all fields optional): { "grossPay":
   ...
   * @param insuranceDeduction Updated insurance deduction (optional, recalculated if grossPay
   *     changes)
   ...
   *
   * <p>DELETE /api/payroll/paycheck/{paycheckId}
   *
   ...
   *
   * <p>PUT /api/payroll/paycheck/{paycheckId}/status?status=PAID
   *
  ...
  public ResponseEntity<PaycheckDTO> updatePaycheckStatus(
      @PathVariable Long paycheckId, @RequestParam PaycheckStatus status) {
