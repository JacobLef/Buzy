...
/**
 * REST Controller for payroll operations Handles payroll calculation and bonus
...
public class PayrollController {
	...
	/**
	 * Constructor injection - Spring provides PayrollService and BusinessService
	 * implementations
	 */
	public PayrollController(PayrollService payrollService, BusinessService businessService,
		...
		if (progressiveTaxStrategy == null) {
			...
			logger.info("PayrollController initialized with progressiveTaxStrategy: {}",
					progressiveTaxStrategy.getStrategyName());
		}
	}
	 ...
	 *
	 * <p>
	 * POST /api/payroll/calculate/{employeeId} POST
	 * /api/payroll/calculate/{employeeId}?additionalPay=1000
	 *
	 * @param employeeId
	 *            ID of the employee
	 * @param additionalPay
	 *            Optional additional amount to add to base salary (bonus, overtime,
	 *            etc.)
	 * @return PaycheckDTO with calculated gross, deductions, and net pay
	 */
	@PostMapping("/calculate/{employeeId}")
	public ResponseEntity<PaycheckDTO> calculatePayroll(@PathVariable Long employeeId,
			@RequestParam(required = false) Double additionalPay) {
		if (additionalPay != null) {
			logger.info("Received payroll calculation request for employee ID: {} with additional pay: ${}", employeeId,
					additionalPay);

			PaycheckDTO dto = payrollService.calculatePayrollWithAdditionalPay(employeeId, additionalPay);

			logger.info("Payroll with additional pay calculated successfully for employee ID: {}", employeeId);
	 ...
	 *
	 * <p>
	 * GET /api/payroll/preview/{employeeId} GET
	 * /api/payroll/preview/{employeeId}?additionalPay=1000
	 *
	 * @param employeeId
	 *            ID of the employee
	 * @param additionalPay
	 *            Optional additional amount to add to base salary (bonus, overtime,
	 *            etc.)
	 * @return PaycheckDTO with calculated gross, deductions, and net pay (not
	 *         saved)
	 */
	@GetMapping("/preview/{employeeId}")
	public ResponseEntity<PaycheckDTO> previewPayroll(@PathVariable Long employeeId,
			@RequestParam(required = false) Double additionalPay) {
		logger.info("Received payroll preview request for employee ID: {} with additional pay: ${}", employeeId,
				additionalPay != null ? additionalPay : 0.0);
	 ...
	 *
	 * <p>
	 * POST /api/payroll/bonuses Request Body: { "businessId": 1, "bonusAmount":
	 * 1000.00, "employeeIds": [1, 2, 3], // Optional: specific employees
	 * "department": "Engineering", // Optional: filter by department "description":
	 * "Q4 Performance Bonus" }
	 *
	 * @param request
	 *            Bonus distribution request with filters
	...
	public ResponseEntity<BonusDistributionResponse> distributeBonuses(
		...
		logger.info("Received bonus distribution request for business ID: {} with amount: ${}", request.businessId(),
				request.bonusAmount());
		...
		BonusDistributionResponse response = BonusDistributionResponse.success(request.businessId(), businessName,
				paycheckDTOs);

		logger.info("Bonus distribution completed successfully: {} paychecks generated", paycheckDTOs.size());
	 ...
	 *
	 * <p>
	 * PUT /api/payroll/tax-strategy?strategy=flatTaxStrategy PUT
	 * /api/payroll/tax-strategy?strategy=progressiveTaxStrategy
	 *
	 * @param strategy
	 *            Strategy name: "flatTaxStrategy" or "progressiveTaxStrategy"
	 * @return TaxStrategySwitchResponse DTO with success message and strategy name,
	 *         or error message
	 */
		...
		try {
			...
			switch (strategy.toLowerCase()) {
				...
				case "progressive" :
					...
					break;
				default :
					return ResponseEntity.badRequest().body(TaxStrategySwitchResponse
							.error("Invalid strategy. Use 'flatTaxStrategy' or 'progressiveTaxStrategy'"));
			}

			payrollService.setTaxStrategy(selectedStrategy);

			TaxStrategySwitchResponse response = TaxStrategySwitchResponse.success("Tax strategy updated successfully",
	...
	public ResponseEntity<List<PaycheckDTO>> getPayrollHistory(@PathVariable Long employeeId) {
	...
	public ResponseEntity<List<PaycheckDTO>> getPayrollHistoryByDateRange(@PathVariable Long employeeId,
			@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate,
			@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate endDate) {
		logger.info("Fetching payroll history for employee ID: {} from {} to {}", employeeId, startDate, endDate);

		List<PaycheckDTO> history = payrollService.getPayrollHistory(employeeId, startDate, endDate);

		logger.info("Retrieved {} paychecks for employee ID: {} in date range", history.size(), employeeId);
	...
	public ResponseEntity<PayrollSummaryDTO> getPayrollSummary(@PathVariable Long businessId,
			@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate,
			@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate endDate) {
		...
		logger.info("Payroll summary generated: {} paychecks, {} employees, Total Net: ${}", summary.totalPaychecks(),
				summary.totalEmployees(), summary.totalNetPay());
	 ...
	 *
	 * <p>
	 * PUT /api/payroll/paycheck/{paycheckId} Request Body (all fields optional): {
	 * "grossPay": 55000.0, "bonus": 2000.0, "taxDeduction": 8250.0,
	 * "insuranceDeduction": 2750.0 }
	 *
	 * @param paycheckId
	 *            ID of the paycheck to update
	 * @param grossPay
	 *            Updated gross pay (optional)
	 * @param bonus
	 *            Updated bonus amount (optional)
	 * @param taxDeduction
	 *            Updated tax deduction (optional, recalculated if grossPay changes)
	 * @param insuranceDeduction
	 *            Updated insurance deduction (optional, recalculated if grossPay
	 *            changes)
	 * @return Updated PaycheckDTO
	 */
	@PutMapping("/paycheck/{paycheckId}")
	public ResponseEntity<PaycheckDTO> updatePaycheck(@PathVariable Long paycheckId,
			@RequestParam(required = false) Double grossPay, @RequestParam(required = false) Double bonus,
		...
		PaycheckDTO updated = payrollService.updatePaycheck(paycheckId, grossPay, bonus, taxDeduction,
				insuranceDeduction);
	...
	/**
	 * Delete a paycheck Only allowed if status is DRAFT
	 *
	 * <p>
	 * DELETE /api/payroll/paycheck/{paycheckId}
	 *
	 * @param paycheckId
	 *            ID of the paycheck to delete
	 ...
	 *            ID of the paycheck
	 * @param status
	 *            New status (DRAFT, PENDING, PAID, VOIDED)
	 * @return Updated PaycheckDTO
	 */
	@PutMapping("/paycheck/{paycheckId}/status")
	public ResponseEntity<PaycheckDTO> updatePaycheckStatus(@PathVariable Long paycheckId,
