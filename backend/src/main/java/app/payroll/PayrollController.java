...
/**
 * REST Controller for payroll operations Handles payroll calculation and bonus distribution
 * endpoints
 *
 * @author Qing Mi
 */
...
public class PayrollController {
  ...
  public PayrollController(PayrollService payrollService, BusinessService businessService,
    ...
    if (flatTaxStrategy == null) {
      ...
      logger.info("PayrollController initialized with flatTaxStrategy: {}",
          flatTaxStrategy.getStrategyName());
    }
   ...
   *          ID of the employee
   * @param additionalPay
   *          Optional additional amount to add to base salary (bonus, overtime, etc.)
  ...
  public ResponseEntity<PaycheckDTO> calculatePayroll(@PathVariable Long employeeId,
    ...
    if (additionalPay != null) {
      logger.info(
          "Received payroll calculation request for employee ID: {} with additional pay: ${}",
   ...
   *
   * <p>
   * GET /api/payroll/preview/{employeeId} GET /api/payroll/preview/{employeeId}?additionalPay=1000
   *
   * @param employeeId
   *          ID of the employee
   * @param additionalPay
   ...
   *
   * <p>
   * POST /api/payroll/bonuses Request Body: { "businessId": 1, "bonusAmount": 1000.00,
   * "employeeIds": [1, 2, 3], // Optional: specific employees "department": "Engineering", //
   * Optional: filter by department "description": "Q4 Performance Bonus" }
   *
   * @param request
  ...
  public ResponseEntity<BonusDistributionResponse> distributeBonuses(
    ...
    BonusDistributionResponse response = BonusDistributionResponse.success(request.businessId(),
        businessName, paycheckDTOs);
  ...
  public ResponseEntity<TaxStrategySwitchResponse> switchTaxStrategy(
    ...
    try {
      ...
      TaxStrategySwitchResponse response = TaxStrategySwitchResponse
          .success("Tax strategy updated successfully", selectedStrategy.getStrategyName());
      ...
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
          TaxStrategySwitchResponse.error("Failed to switch tax strategy: " + e.getMessage()));
    }
  }
  ...
  public ResponseEntity<List<PaycheckDTO>> getPayrollHistoryByDateRange(
    ...
    logger.info("Fetching payroll history for employee ID: {} from {} to {}", employeeId, startDate,
        endDate);

    List<PaycheckDTO> history = payrollService.getPayrollHistory(employeeId, startDate, endDate);

    logger.info("Retrieved {} paychecks for employee ID: {} in date range", history.size(),
        employeeId);
   ...
   *
   * <p>
   * PUT /api/payroll/paycheck/{paycheckId} Request Body (all fields optional): { "grossPay":
   * 55000.0, "bonus": 2000.0, "taxDeduction": 8250.0, "insuranceDeduction": 2750.0 }
   *
   * @param paycheckId
   ...
   *          Updated insurance deduction (optional, recalculated if grossPay changes)
