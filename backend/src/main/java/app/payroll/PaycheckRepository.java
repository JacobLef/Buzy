...
public interface PaycheckRepository extends JpaRepository<Paycheck, Long> {
  ...
  List<Paycheck> findByEmployeeIdAndDateRange(Long employeeId, LocalDate startDate,
      LocalDate endDate);
  ...
  @Query("SELECT p FROM Paycheck p WHERE p.employee.company.id = :businessId "
      + "AND p.payDate BETWEEN :startDate AND :endDate")
  List<Paycheck> findByBusinessIdAndDateRange(Long businessId, LocalDate startDate,
      LocalDate endDate);
