...
/**
 * DTO for Payroll Summary/Report response
 * Contains aggregated statistics for payroll analysis
 * 
 * @author Qing Mi
 */
public record PayrollSummaryDTO(
    ...
    public PayrollSummaryDTO {
        ...
        // Denominator should be totalGrossPay + totalBonus because deductions are calculated on total compensation
        double totalCompensation = (totalGrossPay != null ? totalGrossPay : 0.0) + (totalBonus != null ? totalBonus : 0.0);
        if (totalCompensation > 0) {
            averageTaxRate = totalTaxDeductions != null ? totalTaxDeductions / totalCompensation : 0.0;
            averageInsuranceRate = totalInsuranceDeductions != null ? totalInsuranceDeductions / totalCompensation : 0.0;
        } else {
            averageTaxRate = 0.0;
