...
public class DashboardServiceImpl {
	...
	public DashboardServiceImpl(PaycheckRepository paycheckRepository, EmployeeRepository employeeRepository,
	...
	public List<ActivityDTO> getRecentActivity(Long businessId) {
		...
		var paychecksByDate = recentPaychecks.stream().collect(Collectors.groupingBy(Paycheck::getPayDate));
		...
		for (var entry : paychecksByDate.entrySet()) {
			...
			activities.add(new ActivityDTO(payDate.toEpochDay(), "PAYROLL", title, formatRelativeTime(payDateTime),
					payDateTime, "completed"));
		}
		...
		for (var employee : recentHires) {
			...
			activities.add(new ActivityDTO(employee.getId(), "EMPLOYEE", "New Hire: " + employee.getName(),
					formatRelativeTime(hireDateTime), hireDateTime, "info"));
		}

		LocalDate weekFromNow = LocalDate.now().plusDays(7);
		var expiringTrainings = trainingRepository.findExpiringBetween(businessId, LocalDate.now(), weekFromNow);
		...
		for (var training : expiringTrainings) {
			...
			activities.add(new ActivityDTO(training.getId(), "TRAINING", training.getTrainingName() + " expires soon",
					"Due in " + daysUntilExpiry + (daysUntilExpiry == 1 ? " day" : " days"), expiryDateTime,
					"warning"));
		}

		return activities.stream().sorted(Comparator.comparing(ActivityDTO::timestamp).reversed()).toList();
	}
