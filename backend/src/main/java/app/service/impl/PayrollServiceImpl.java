...
import app.dto.request.DistributeBonusRequest;
import app.dto.response.PaycheckDTO;
import app.dto.response.PayrollSummaryDTO;
import app.factory.DTOFactory;
import app.exception.BusinessValidationException;
import app.exception.PayrollCalculationException;
import app.exception.ResourceNotFoundException;
import app.model.domain.Company;
import app.model.domain.Employee;
import app.model.domain.Employer;
import app.model.payroll.Paycheck;
import app.model.payroll.PaycheckStatus;
import app.repository.PaycheckRepository;
import app.repository.BusinessRepository;
import app.service.interfaces.PayrollService;
import app.service.interfaces.EmployeeService;
import app.strategy.tax.TaxCalculationStrategy;
...
/**
 * Implementation of PayrollService handling all payroll calculations
 * Uses Strategy pattern for tax calculations and supports multiple deduction types
 * 
 * @author Qing Mi
 */
@Service
public class PayrollServiceImpl implements PayrollService {
     ...
     * Insurance rate is configurable via application.properties with default value 0.05 (5%)
     */
    @Autowired
     ...
     * Logs the tax strategy and insurance rate being used
     */
    @PostConstruct
    public void init() {
        if (taxStrategy == null) {
    ...
    public PaycheckDTO calculatePayrollWithAdditionalPay(Long employeeId, Double additionalPay) {
        ...
        logger.debug("Calculating payroll for employee ID: {} with additional pay: {}", 
    ...
    public List<PaycheckDTO> distributeBonuses(DistributeBonusRequest request) {
        ...
        if (employees.isEmpty()) {
            throw new BusinessValidationException(
                "No employees found matching the criteria for bonus distribution");
        }
        ...
        for (Employee employee : employees) {
            try {
                ...
                logger.debug("Bonus paycheck created for employee {}: Base=${}, Bonus=${}, Net=${}", 
                    employee.getName(), savedPaycheck.getGrossPay(), 
                    savedPaycheck.getBonus(), savedPaycheck.getNetPay());
        ...
        if (paycheckDTOs.isEmpty() && !errors.isEmpty()) {
            throw new PayrollCalculationException(
                "Failed to distribute bonuses to any employees. Errors: " + 
                String.join("; ", errors), 
                request.businessId()
            );
        }
        
        logger.info("Bonus distribution completed: {} successful, {} failed", 
            paycheckDTOs.size(), errors.size());
    ...
    public void setTaxStrategy(TaxCalculationStrategy strategy) {
        ...
        logger.info("Tax strategy changed from '{}' to '{}'", 
    ...
    private Paycheck calculateRegularPayroll(Employee employee) {
        ...
        if (employee.getSalary() == null || employee.getSalary() < MINIMUM_SALARY_THRESHOLD) {
            throw new PayrollCalculationException(
                "Employee has invalid salary: " + employee.getSalary(),
                employee.getId()
            );
        }
        ...
        logger.debug("Regular payroll calculated - Base Salary: ${}, Tax: ${}, Insurance: ${}, Net: ${}",
    ...
    /**
     * Calculate payroll with bonus payment
     * In real-life: bonus is separate from base salary, taxed on total (base + bonus)
     * 
     * @param employee Employee to calculate payroll for
    ...
    private Paycheck calculatePayrollWithBonus(Employee employee, Double bonusAmount) {
        ...
        logger.debug("Payroll with bonus calculated - Base: ${}, Bonus: ${}, Total Gross: ${}, Tax: ${}, Insurance: ${}, Net: ${}",
    ...
    /**
     * Calculate insurance deduction based on gross pay
     * Uses configurable insurance rate from application.properties
     * Can be extended to use Strategy pattern if needed
     * 
     ...
     */
    private List<Employee> fetchEmployeesForBonus(
        DistributeBonusRequest request, 
        Company business
    ) {
        ...
        if (request.employeeIds() != null && !request.employeeIds().isEmpty()) {
            ...
            allEmployees = request.employeeIds().stream()
    ...
    public List<PaycheckDTO> getPayrollHistory(Long employeeId, LocalDate startDate, LocalDate endDate) {
        logger.debug("Fetching payroll history for employee ID: {} from {} to {}", 
        ...
        List<Paycheck> paychecks = paycheckRepository.findByEmployeeIdAndDateRange(
            employeeId, startDate, endDate);
    ...
    public PayrollSummaryDTO getPayrollSummary(Long businessId, LocalDate startDate, LocalDate endDate) {
        ...
        List<Paycheck> paychecks = paycheckRepository.findByBusinessIdAndDateRange(
            businessId, startDate, endDate);
        ...
        long totalEmployees = paychecks.stream()
            .map(p -> p.getEmployeeId())
            .distinct()
            .count();
        
        double totalGrossPay = paychecks.stream()
            .mapToDouble(Paycheck::getGrossPay)
            .sum();
        
        double totalBonus = paychecks.stream()
            .mapToDouble(p -> p.getBonus() != null ? p.getBonus() : 0.0)
            .sum();
        
        double totalTaxDeductions = paychecks.stream()
            .mapToDouble(Paycheck::getTaxDeduction)
            .sum();
        
        double totalInsuranceDeductions = paychecks.stream()
            .mapToDouble(Paycheck::getInsuranceDeduction)
            .sum();
        
        double totalDeductions = totalTaxDeductions + totalInsuranceDeductions;
        
        double totalNetPay = paychecks.stream()
    ...
    public PaycheckDTO updatePaycheck(Long paycheckId, Double grossPay, Double bonus, 
        ...
        logger.info("Paycheck ID: {} updated successfully. New net pay: ${}", 
            paycheckId, updatedPaycheck.getNetPay());
