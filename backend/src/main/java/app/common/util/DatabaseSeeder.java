...
public class DatabaseSeeder implements CommandLineRunner {
	...
	public DatabaseSeeder(BusinessRepository businessRepo, EmployeeRepository employeeRepo,
			...
			BusinessPersonRepository businessPersonRepo, UserRepository userRepo, PaycheckRepository paycheckRepo,
	...
	private void seedCompanies() {
		...
		try {
			BufferedReader reader = new BufferedReader(
					new InputStreamReader(new ClassPathResource(CSV_PREFIX + "/businesses.csv").getInputStream()));
	...
	private Company mapToCompany(Map<String, String> row) {
		return Company.builder().name(CONVERTER.toString(row.get("name")))
				.address(CONVERTER.toString(row.get("address"))).industry(CONVERTER.toString(row.get("industry")))
	...
	private void seedEmployers() {
		...
		try {
			BufferedReader reader = new BufferedReader(
					new InputStreamReader(new ClassPathResource(CSV_PREFIX + "/employers.csv").getInputStream()));
				...
				for (Map<String, String> row : rows) {
					...
					if ((isOwnerFromCSV == null || !isOwnerFromCSV) && companyId != null) {
						if (!companyOwnerSet.containsKey(companyId)) {
							...
							if (title != null
									&& (title.toLowerCase().contains("ceo") || title.toLowerCase().contains("chief")
								...
								System.out.println(
										"Auto-set " + employer.getName() + " as Owner for company " + companyId);
							} else {
								// Set first employer as owner if no CEO/Chief found
								employer.setIsOwner(true);
								employer.setIsAdmin(true);
								companyOwnerSet.put(companyId, true);
								System.out.println("Auto-set first employer " + employer.getName()
										+ " as Owner for company " + companyId);
							}
						}
	...
	private Employer mapToEmployer(Map<String, String> row) {
		Employer employer = new Employer(CONVERTER.toString(row.get("name")), CONVERTER.toString(row.get("email")),
	...
	private void seedEmployees() {
		...
		try {
			BufferedReader reader = new BufferedReader(
					new InputStreamReader(new ClassPathResource(CSV_PREFIX + "/employees.csv").getInputStream()));
				...
				for (Map.Entry<Long, Long> entry : managerRelationships.entrySet()) {
					...
					if (employee != null) {
						...
						if (manager != null) {
							...
							System.err.println("Warning: Manager not found for employee ID " + employeeId
	...
	private Employee mapToEmployee(Map<String, String> row) {
		Employee employee = new Employee(CONVERTER.toString(row.get("name")), CONVERTER.toString(row.get("email")),
				CONVERTER.toString(row.get("password")), CONVERTER.toDouble(row.get("salary")),
	...
	private void seedTrainings() {
		...
		try {
			BufferedReader reader = new BufferedReader(
					new InputStreamReader(new ClassPathResource(CSV_PREFIX + "/trainings.csv").getInputStream()));
			...
			if (rows != null && !rows.isEmpty()) {
				...
				for (Map<String, String> row : rows) {
					Training training = new Training(CONVERTER.toString(row.get("training_name")),
							...
							CONVERTER.toLocalDate(row.get("expiry_date")), CONVERTER.toBoolean(row.get("is_required")));
				...
				System.out
						.println("Seeded " + count + " trainings" + (skipped > 0 ? " (skipped " + skipped + ")" : ""));
			}
		} catch (Exception e) {
	...
	private void seedPaychecks() {
		...
		try {
			BufferedReader reader = new BufferedReader(
					new InputStreamReader(new ClassPathResource(CSV_PREFIX + "/paychecks.csv").getInputStream()));
			...
			if (rows != null && !rows.isEmpty()) {
				...
				System.out
						.println("Seeded " + count + " paychecks" + (skipped > 0 ? " (skipped " + skipped + ")" : ""));
			}
		} catch (Exception e) {
