...
public class DTOFactory {
	...
	public EmployeeDTO createDTO(Employee employee) {
		EmployeeDTO.Builder builder = EmployeeDTO.builder().withId(employee.getId()).withName(employee.getName())
				.withEmail(employee.getEmail()).withStatus(employee.getStatus()).withSalary(employee.getSalary())
		...
		if (employee.getCompany() != null) {
			builder.withCompanyId(employee.getCompany().getId()).withCompanyName(employee.getCompany().getName());
		}

		if (employee.getManager() != null) {
			builder.withManagerId(employee.getManager().getId()).withManagerName(employee.getManager().getName());
		}
	...
	public EmployerDTO createDTO(Employer employer) {
		EmployerDTO.Builder builder = EmployerDTO.builder().withId(employer.getId()).withName(employer.getName())
				.withEmail(employer.getEmail()).withStatus(employer.getStatus()).withSalary(employer.getSalary())
		...
		if (employer.getCompany() != null) {
			builder.withCompanyId(employer.getCompany().getId()).withCompanyName(employer.getCompany().getName());
		}
	...
	public CompanyDTO createDTO(Company company) {
		...
		return CompanyDTO.builder().withId(company.getId()).withName(company.getName())
				...
				.withTotalEmployers(company.getEmployersOnly().size()).withTotalPersons(company.getTotalPersonsCount())
				.withEmployeeIds(employeeIds).withEmployerIds(employerIds).build();
	}
	...
	public PaycheckDTO createDTO(Paycheck paycheck, Employee employee, String taxStrategyName) {
		...
		return new PaycheckDTO(paycheck.getId(), paycheck.getEmployeeId(), employee.getName(), paycheck.getGrossPay(), // Base
																														// salary
				paycheck.getBonus(), // Bonus amount (null if regular payroll)
				paycheck.getTaxDeduction(), paycheck.getInsuranceDeduction(), totalDeductions, paycheck.getNetPay(),
				paycheck.getPayDate(), taxStrategyName, paycheck.getStatus() // Status: DRAFT, PENDING, PAID, VOIDED
		);
	}
}
