...
public class SignupServiceImpl implements SignupService {
	...
	public AuthDTO signupEmployee(CreateEmployeeRequest request) {
		...
		CreateEmployeeRequest requestWithEncryptedPassword = new CreateEmployeeRequest(request.name(), request.email(),
				encryptedPassword, // already encrypted, for employeeService use
				request.salary(), request.position(), request.companyId(), request.managerId(), request.hireDate());
	...
	public AuthDTO signupEmployer(CreateEmployerRequest request) {
		...
		CreateEmployerRequest requestWithEncryptedPassword = new CreateEmployerRequest(request.name(), request.email(),
				encryptedPassword, // Already encrypted, EmployerService will use it directly
				request.salary(), request.department(), request.title(), request.companyId(), request.hireDate());
